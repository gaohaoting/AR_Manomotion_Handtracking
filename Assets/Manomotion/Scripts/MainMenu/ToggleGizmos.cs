...
public class ToggleGizmos : MonoBehaviour
{
    ...
    /// <summary>
    /// Toggles the boolean value of running one or two hands.
    /// By toggling the value it will also update the manomotion session to start calculate one or two hands.
    /// </summary>
    public void ToggleRunTwoHands()
    {
        _gizmoManager.TwoHands = !_gizmoManager.TwoHands;
        ManomotionManager.Instance.ShouldRunTwoHands(_gizmoManager.TwoHands);

        if (ManomotionManager.Instance.ManomotionSession.enabledFeatures.skeleton3D == 0)
        {
