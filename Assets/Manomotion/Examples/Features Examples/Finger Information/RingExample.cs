...
public class RingExample : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        if (fingerInfoGizmo == null)
        {
            try
            {
                fingerInfoGizmo = GizmoManager.Instance.fingerInfo;

            }
            catch 
            {
                Debug.Log("Cant find finger GameObject");
            }
        }
    ...
    void Update()
    {
        ...
        if (gestureInfo.manoClass == ManoClass.GRAB_GESTURE)
        {
            fingerInfoGizmo.ShowFingerInformation(ManomotionManager.Instance.HandInfos[0].trackingInfo, gestureInfo);
            ShowRing();
        }
    ...
    private void ShowRing()
    {
        ...
        float distanceBetweenKnuckles = Vector3.Distance(ManomotionManager.Instance.HandInfos[0].trackingInfo.skeleton.jointPositions[5], ManomotionManager.Instance.HandInfos[0].trackingInfo.skeleton.jointPositions[17]);
        ...
        transform.localScale = new Vector3(distanceBetweenKnuckles, distanceBetweenKnuckles, distanceBetweenKnuckles);
